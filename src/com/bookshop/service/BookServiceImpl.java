 ...
 * 
 */
public class BookServiceImpl extends MyServiceParentImpl implements BookService {
  @Resource
  ...
  public boolean increaseStorageNum(long bookId, int storageNum) {
    this.session = getSession();
    hql = "from Book as b where b.book_id='" + bookId + "'";
    Book b = (Book) bookDao.find(hql, session);
    if (b != null) {
      b.setStorage_num(b.getStorage_num() + storageNum);
      bookDao.update(b, session);
      return true;
    } else{
      return false;
  ...
  public void decrementStorageNum(long bookId, int decrement) {
    this.session = getSession();
    hql = "from Book as b where b.book_id='" + bookId + "'";
    Book b = (Book) bookDao.find(hql, session);
    if (b == null) {
    ...
    int storageNum = b.getStorage_num() - decrement;
    if (storageNum >= 0) {
      b.setStorage_num(storageNum);
    }else {
      throw new RuntimeException();
    }
    bookDao.update(b, session);
