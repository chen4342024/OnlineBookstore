...
import com.bookshop.dao.MyDaoParentImpl;

public class MyServiceParentImpl implements MyServiceParent{
	@Resource
	private SessionFactory sessionFactory;
	@Resource
	public MyDaoParentImpl myDaoParent;
	...
	public Long hasNumbers(String entityName, int status) {
		...
		String hql;
		hql = "select count(*) from " + entityName
				+ " as entity where entity.status =" + status;
		return myDaoParent.findCount_all(session, hql);
	}
	...
	public List<?> show_by_page(int page, int perFolioAmount,
		...
		this.session = this.getSession();
		String hql;
		hql = "from " + entityName + " as entity where entity.status ='"
				+ status + "'";
		return myDaoParent.show_by_page_all(page, perFolioAmount, session, hql);
	}
	...
	public List<?> show_by_page(int page, int perFolioAmount,
		...
		if (paramName == null) {
			return myDaoParent.show_by_page_all(page, perFolioAmount, session,
					hql);
		}
		if (paramName.length != value.length) {
			return null;
	...
	public Long hasNumbers(String entityName, String timeParam, String begin,
		...
		this.session = getSession();
		String hql = "select count(*) from " + entityName;
		hql = hql + " as t where t." + timeParam + " between '" + begin
				+ "'and '" + end + "'";
		if (parm != null && !parm.trim().equals("") && value != null
